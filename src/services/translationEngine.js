...
1. ISOLATED BOX (MOST CRITICAL): Each <s id="N"> is sealed. Translate ONLY its content in TOTAL ISOLATION. Fragment IN = Fragment OUT.
...
2. ESCAPE HATCH: Cannot translate, or line contains only symbols/music notes/song lyrics (♪) → copy EXACT ORIGINAL TEXT for that ID. Never shift remaining entries.

3. STRUCTURE & COUNT: Output EXACTLY ${expectedCount} entries (ID_${startId} to ID_${endId}). IDs must match input exactly — never invent, skip, or fill gaps.

4. PRESERVE FORMATTING: Preserve ALL inline tags ([br], <i>, etc.) and speaker dashes (-) EXACTLY as they appear in the source text.

5. CLEAN OUTPUT: Format: <s id="45">text</s>. Start response immediately with first <s> tag. Zero preamble, markdown, or commentary. Nothing floating outside tags.
