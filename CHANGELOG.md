...
- **Fixed tool-page cache-buster asset redirects and the History page service worker registration being rejected by the browser:** `sub-toolbox`, `file-upload`, `auto-subtitles`, and `sub-history` were still emitting some shared UI assets (`sw-register.js`, `theme-toggle.js`, `subtitle-menu.js`, `combobox.css`, and toolbox favicons) without the `_cb` cache-buster query that the middleware expects. Most of those pages still appeared to work because the browser followed the resulting `307` redirects, but they were paying extra asset hops on every load and relying on redirect behavior for core JS/CSS. The remaining real runtime failure was on `/sub-history`: `public/js/sw-register.js` still tried to register `/sw.js` without `_cb`, and browsers reject redirected service-worker scripts, so the page logged `The script resource is behind a redirect, which is disallowed.` and skipped service-worker registration on that load. Tool and history page generators now emit direct cache-busted asset URLs, `sw-register.js` registers `/sw.js?...&_cb=...` directly, and added regression coverage now compiles the affected page runtimes and asserts those cache-busted URLs stay direct.
